...
###################################################################
  ...
  # ---------
      ...
      vias_mgn_raster <- shp2raster(shp = vias_mgn, mask.raster = list_raster[[1]], label = "Vias Rasterizadas", transform = FALSE, value = 1)
      writeRaster(rp, filename = "vias_mgn_raster_1km_2.tif", format = "GTiff", progress = "text", overwrite = T)  
      
        
        rp <- rasterize(vias_mgn, list_raster[[1]], vias_mgn@data$DPTO_NAREA)
        plot(rp)
        writeRaster(rp, filename = "vias_mgn_raster_1km.tif", format = "GTiff", progress = "text", overwrite = T)  
