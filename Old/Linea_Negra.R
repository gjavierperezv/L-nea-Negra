...
# -----------------------------------------------
  ...
  # ------------------------
    ...
    linea_negra <- readOGR(".", "Linea_Negra_Polygon") 
    linea_negra_proj <- spTransform(linea_negra, CRS=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
    #plot(linea_negra_proj)

# 2. RASTERS PROCESSING: crop, mask, make calculations and extract raster data
#-----------------------------------------------------------------------------
...
rasters_lights <- processing_rasters(list_raster, rasters_extent, mpios_ln_proj)
#plot(rasters_lights)
...
#----------------------
  ...
  # ----------------------------------------------------------------------
    # Oppening Colombia's Shape File  (NOTE:OPEN THIS FILE BEFORE DE RASTERS)
      ...
      loss_year <- lapply(list.files()[files], raster) %>%
        lapply(crop, colombia_mpios_proj) #lapply returns a list of the same length as X, each element of which is the result of applying FUN to the corresponding element of X.
  ...
  # --------------------------------------------
    ...
    # -------------------------------
      ...
      # Map
      setwd("~/Dropbox/Linea_Negra_R/Data/WDPA")
      loss_year_mosaic_30m <- raster("loss_year_mosaic_30m.tif", overwrite = TRUE)
      #plot(loss_year_mosaic_30m)
      
      #setwd("~/Dropbox/Linea_Negra_R/Data/Colombia_Municipios")
      #colombia_mpios <- readOGR(".", "colombia_mpios") 
      #plot(colombia_mpios, add = T)
    ...
    # Refference: https://github.com/ivanhigueram/deforestacion/blob/master/Extracting/raster_loss.R
      ...
      setwd("~/Dropbox/Linea_Negra_R/Data/WDPA")
      loss_year_brick <-  raster("loss_year_brick.tif", overwrite = TRUE) 
      ...
      agg <- raster("loss_year_brick_agg.tif", overtwrite = T)
      
      agg_mask <- mask(agg, colombia_mpios_proj)
      ...
      res <- resample(agg_mask, rasters_lights, filename = "loss_year_brick_1km.tif",  # Resample: transfers values between non matching Raster* objects (in terms of origin and resolution).
                      ...
                      overwrite = T)
      # Lectura de archivo ya existente
      res <- raster("loss_year_brick_1km.tif", overwrite = T)
  ...
  # --------------------------------------------------------------------
      res <- raster("loss_year_brick_1km.tif")
...
#----------------  
  ...
  rasters_soil <- processing_rasters(rasters, rasters_extent, mpios_ln_proj) %>%
...
# 7. SLOPE AND ASPECTS
# --------------------
  slope_mpios_ln <- terrain(elevation, opt = "slope")
  aspect_mpios_ln <- terrain(elevation, opt = "aspect")
  hills_mpios_ln <- hillShade(slope_mpios_ln, aspect_mpios_ln, angle = 40, 0)
  roughness_mpios_ln <- terrain(elevation, opt = "roughness")
  flowdir_mpios_ln <- terrain(elevation, opt = "flowdir")
  names(hills_mpios_ln) <- "hill"
  #Plots
  plot(slope_mpios_ln)
  plot(aspect_mpios_ln)
  plot(hills_mpios_ln)
  plot(roughness_mpios_ln)
  plot(flowdir_mpios_ln)
...
# ----------------
  ...
  # ---------------------------------------------------------------------------------------------------------------------
    clump_lights <- clump(rasters_lights[[2]], directions = 8) %>%  # Detect clumps (patches) of connected cells. Each clump gets a unique ID. NA and zero are used as background values (i.e. these values are used to separate clumps). You can use queen's or rook's case, using the directions argument.
      resample(., res_ln[[1]])     # Resample transfers values between non matching Raster* objects (in terms of origin and resolution)
...
# -----------------
  ...
  # 10.2. GET CLIMATE
  # -----------------
  # ES ESTA LA FUENTE DE LOS DATOS?: https://www.esrl.noaa.gov/psd/data/gridded/data.prec.html
  # o ES ESTA?: https://www.esrl.noaa.gov/psd/data/gridded/data.UDel_AirT_Precip.html
  ...
  setwd("~/Dropbox/Linea_Negra_R/Data/FAO")
  ...
  sq <- lapply(list_files, raster) %>%
    ...
    crop(mpios_ln_proj) %>%
    mask(mpios_ln_proj) %>%
    resample(res_ln[[1]], filename = "sq_1km_ln_tif",
             format = "GTiff",
             options = "INTERLEAVE = BAND",
             progress = "text", overwrite = T)
  #list(sq)
  #plot(sq)
  sq <- raster("sq_1km_ln_tif.tif")
  #plot(sq)
  ...
  buffer_roads <- gBuffer(roads_deptos_ln, 5000, byid = TRUE, id = row.names(roads_deptos_ln)) %>%
    spTransform(CRS=CRS("+proj=longlat +datum=WGS84 +no_defs +ellps=WGS84 +towgs84=0,0,0"))
  plot(buffer_roads)
  ...
  # --------------------------------
  dataframes_extract <- list(elevation, slope, roughness, tri, clump_lights, sq) %>%  # Falta inlcuir precipitacion prec
...
###################################
# ESTO ES LO QUE FALTA DE CLUMPS:
###################################
