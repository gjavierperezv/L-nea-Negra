...
#############################################################################################
  ...
  # ----------------------------------
    # Leyendo la infoamción de Luces Nocturnas
      ...
      a <- as.data.frame(rasters_lights[[1]], xy = T, na.rm = T, centroids = T)
      a$ID <- row.names(a)
      
      b <- setExtent(rasters_lights[[1]], res[[1]]) %>% as.data.frame(xy = T, na.rm = T, centroids = T)
      b$ID <- row.names(b)
      
      c <- as.data.frame(res[[1]], xy = T, na.rm = T)
      c$ID <- row.names(c)
  ...
  # ---------
      ...
      # Referencia: http://gis.stackexchange.com/questions/138861/calculating-road-density-in-r-using-kernel-density
        ...
        psp_vias_mgn <- as.psp(vias_mgn)
        plot(psp_vias_mgn)
        # Pixellate with resolution
        px_vias_mgn <- pixellate(psp_vias_mgn)
        plot(px_vias_mgn)
        # This can be converted to raster as desired
        rLength <- raster(px_vias_mgn)
        # Values:
